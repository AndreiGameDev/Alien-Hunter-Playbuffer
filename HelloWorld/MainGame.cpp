...
void UpdatePlayer() { // Player Controller
	...
	if (playerProperties.canShoot == false && playerProperties.projectileTimer > playerProperties.projectileFireRate) {
		// If the timer is off cd the player can shoot and it resets back to 0
		playerProperties.canShoot = true;
		playerProperties.projectileTimer = 0;
	}
	// Move the player based off velocity
	player.pos += player.velocity;
	if (Play::IsLeavingDisplayArea(player, Play::HORIZONTAL)) { // Keep the player in bounds
		player.pos.x = player.oldPos.x;
	}
	if (Play::KeyPressed(0x01) && playerProperties.canShoot) { // Shoots with mouse 1
	...
	if (Play::KeyDown(0x44)) { // Moves with right
	...
	else if (Play::KeyDown(0x41)) { // Moves with Left
		Play::SetSprite(player, "Player_Spaceship_Moving", playerProperties.animatorSpeed);
		player.velocity.x = -playerProperties.playerSpeed;
	}
	else { // No input
...
void EnemySpawner() { // Spawns Enemies
	...
	if (enemyManager.spawnTimer > enemyManager.spawnRate && enemyManager.enemiesToSpawn > 0) {
		//Reset the timer to 0 and decrease enemies left to spawn
	...
	else if (enemyManager.enemiesToSpawn == 0) { // If no more enemies are there to spawn, trigger a new wave event
...
void NewWaveEvent(float elapsedTime) { // Ramps up difficulty and makes enemies spawn again when a wave dies
	...
	else if (enemyManager.triggerNewWaveCountdown == true) { // Increases timer
		enemyManager.newWaveTimer += elapsedTime;
	}
}
void UpdateEnemies() { // Enemy Controllers
	//Grabs every type of enemy
	...
	// Create the overall array of enemies and append the information from the previous ones
	...
	for (int _idEnemy : vEnemies) { 
		...
		if (Play::IsLeavingDisplayArea(obj_enemy, Play::HORIZONTAL)) { // Keep enemy in bounds and make it drop down if it reaches the boundary
		...
		if (obj_enemy.pos.y >= 200.0f) { // If it reaches y 200+ then destroy it and harm the player
...
void UpdateProjectiles() { // Projectile Logic
	...
	for (int id_laser : vPlayerProjectiles) {
		...
		// If projectile collides with any of the enemy types then it collided
		...
		// If it collided then grab the enemy it collided with and destroy it, also increment the score
		...
		if (!Play::IsVisible || hasCollided) { // If out of bounds or collided then destroy it
